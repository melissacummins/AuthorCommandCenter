-- ============================================
-- UPSELLS: BxGy GET-QUANTITY = 1
-- A "Buy X, Get Y" discount only applies once the cart holds the full
-- "customer gets" quantity. We were setting that quantity to the number of
-- add-ons in the offer, so a shopper who picked just one add-on never met the
-- threshold and got no discount. The add-on discount must apply when the
-- shopper picks ANY one add-on, so the "get" quantity is always 1 — each
-- eligible add-on in the cart takes the discount, and it still drops the
-- instant the main product leaves the cart.
-- This recreates shopify_proxy with create_bxgy_discount's quantity fixed to 1.
    ...
    v_body := jsonb_build_object(
      ...
      'variables', jsonb_build_object(
        'bxgyCodeDiscount', jsonb_build_object(
          ...
          'customerGets', jsonb_build_object(
            -- Quantity is always 1: the discount fires as soon as the shopper
            -- picks any single add-on (each eligible add-on in the cart takes
            -- the discount), instead of requiring all of them to be present.
