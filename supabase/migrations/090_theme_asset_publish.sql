-- ============================================
-- UPSELLS: ONE-CLICK WIDGET PUBLISH
-- shopify_proxy gains two theme actions so the app can publish the
-- storefront widget as a theme snippet (snippets/acc-addons.liquid).
-- The theme's Custom Liquid blocks then contain a single, never-changing
-- line ({% render 'acc-addons' %}) and widget updates become one click
-- in the app instead of a manual re-paste per template.
--   - get_themes: list themes (to find the live/main theme id)
--   - set_theme_asset: upsert the snippet file (key is locked to the
--     app's own snippet so this can't touch other theme files)
-- Requires the connection to be re-authorized with the write_themes scope.
