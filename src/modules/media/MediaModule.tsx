    ...
    <div className="p-4 lg:p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6">
        ...
        <div className="bg-surface rounded-card border border-edge p-5 h-fit space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button onClick={() => setCustomModelsDrawerOpen(true)} className="text-xs text-brand-600 hover:text-brand-700 flex items-center gap-1">
            ...
            {isIdeogramDirectModel && (
              <p className="text-[11px] mt-1">
                {ideogramProvider === 'ideogram' ? (
                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-brand-50 text-brand-700 font-medium">
          ...
          <div>
            ...
            {!describeImageUrl ? (
              <label className={`flex flex-col items-center justify-center gap-1.5 px-3 py-4 rounded-control border-2 border-dashed text-xs cursor-pointer transition-colors ${describing ? 'border-brand-300 bg-brand-50 text-brand-700' : 'border-edge text-content-secondary hover:border-brand-300 hover:bg-brand-50/40'}`}>
                {describing ? (
                  <>
              ...
              <div className="rounded-control border border-edge bg-surface-hover p-2 space-y-2">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="px-2.5 py-1 rounded-control bg-brand-600 text-brand-fg text-[11px] font-semibold hover:bg-brand-700 disabled:opacity-40"
                  >
                    Use as prompt
                  </button>
                  <button
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button onClick={() => setCollectionsDrawerOpen(true)} className="text-xs text-brand-600 hover:text-brand-700 flex items-center gap-1">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-card bg-gradient-to-r from-brand-500 to-brand-600 text-white font-semibold shadow-lg shadow-brand-500/25 hover:shadow-brand-500/40 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div>
          ...
          {selectedIds.size > 0 && (
            <div className="sticky top-0 z-10 flex flex-wrap items-center gap-2 mb-3 bg-brand-50 border border-brand-200 rounded-card px-3 py-2 text-sm">
              <span className="font-semibold text-brand-900">{selectedIds.size} selected</span>
              <select
                value=""
                onChange={(e) => handleBulkMoveToCollection(e.target.value || null)}
                className="text-xs px-2 py-1.5 rounded-control border border-brand-200 bg-surface text-content"
              >
              ...
              <button
                ...
                className="ml-auto text-xs text-brand-700 hover:text-brand-900 underline"
              >
                Clear selection
              </button>
            </div>
...
function MediaCard({
  ...
  return (
    <div className={`bg-surface rounded-card border overflow-hidden flex flex-col ${selected ? 'border-brand-500 ring-2 ring-brand-200' : 'border-edge'}`}>
        ...
        {!pending && !failed && (
          <button
            ...
            className={`absolute top-2 left-2 z-10 w-6 h-6 rounded-control border-2 flex items-center justify-center transition-all ${
              selected
                ? 'bg-brand-600 border-brand-600 text-brand-fg'
                : 'bg-surface/80 border-white hover:border-brand-400'
            }`}
        ...
        {!pending && !failed && generation.output_url && (
          generation.kind === 'video' ? (
            ...
            <button
              ...
              className="w-full h-full block focus:outline-none focus:ring-2 focus:ring-brand-500"
  ...
  return (
    <Drawer title="Custom models" onClose={onClose}>
      <p className="text-sm text-content-secondary mb-4">
        ...
        <a href="https://fal.ai/models" target="_blank" rel="noreferrer" className="text-brand-600 underline inline-flex items-center gap-0.5">
        ...
        {models.map((m) => (
          <div key={m.id} className="border border-edge rounded-card p-3">
            ...
            <div className="flex flex-wrap gap-1.5 mt-1.5 text-[11px]">
              ...
              {m.accepts_input_image && <span className="px-1.5 py-0.5 rounded bg-brand-50 text-brand-700">input image</span>}
