  ...
  if (books.length === 0) {
    return (
      <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-slate-300">
        ...
        <p className="text-sm text-slate-500">Add a book to start seeing your catalog overview.</p>
      </div>
    );
  }
              ...
              upcoming.map((e, i) => (
                ...
                >
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-slate-800 truncate">{e.book.title}</div>
                    <div className="text-xs text-slate-500">{e.label} · {formatDate(e.date)}</div>
                  </div>
                  <span className={`text-xs whitespace-nowrap font-medium ${e.daysAway < 0 ? 'text-rose-600' : e.daysAway <= 14 ? 'text-amber-600' : 'text-slate-500'}`}>
                    {relativeDate(e.date)}
                  </span>
                </button>
              ))
          ...
          <div className="bg-white rounded-2xl border border-slate-200 divide-y divide-slate-100">
            {awaiting.length === 0 ? (
              ...
              awaiting.map((a, i) => (
                ...
                >
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-slate-800 truncate">{a.book.title}</div>
            ...
            {wips.map(b => {
              ...
              return (
                <button
                  ...
                  className="text-left bg-white rounded-xl border border-slate-200 p-4 hover:shadow-md"
                >
                  ...
                  {b.current_chapter && (
                    <div className="text-xs text-slate-500 mb-2">Chapter: {b.current_chapter}</div>
                  )}
                  {target > 0 ? (
                    <>
      ...
      {series.length > 0 && (
        <section>
          ...
          <div className="space-y-3">
            {series.map(group => (
              <div key={group.series} className="bg-white rounded-2xl border border-slate-200 p-4">
                <div className="font-semibold text-slate-800 mb-2">{group.series}</div>
                <div className="space-y-1">
                  {group.books.map(b => (
                    <button
                      key={b.id}
                      onClick={() => onOpenBook(b)}
                      className="w-full text-left flex items-center justify-between gap-3 px-2 py-1.5 rounded hover:bg-slate-50"
                    >
...
function EmptyRow({ text }: { text: React.ReactNode }) {
  return <div className="px-4 py-6 text-center text-sm text-slate-500">{text}</div>;
}

const sectionLabel = 'text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2';
