    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-2xl font-bold text-slate-800">{trope.name}</h2>
            {trope.description && <p className="text-sm text-slate-500">{trope.description}</p>}
          </div>
          <div>
            <input
              ref={fileInputRef}
            ...
            <button
              ...
              disabled={busy}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 rounded-lg shadow-sm"
            >
              <Upload className="w-4 h-4" /> Upload Publisher Rocket CSV
            </button>
          </div>
        </div>
        {error && <div className="mt-3 p-3 rounded-lg bg-rose-50 border border-rose-200 text-sm text-rose-700">{error}</div>}
        {info && <div className="mt-3 p-3 rounded-lg bg-emerald-50 border border-emerald-200 text-sm text-emerald-800">{info}</div>}
      </div>
      ...
      {selected.size > 0 && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-3 flex flex-wrap items-center gap-3 text-sm">
          ...
          <button
            ...
            disabled={busy}
            className="inline-flex items-center gap-1 px-3 py-1.5 text-rose-700 bg-white border border-rose-200 hover:bg-rose-50 rounded-lg"
          >
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-1.5 text-indigo-700 bg-white border border-indigo-200 hover:bg-indigo-50 disabled:opacity-50 rounded-lg"
            >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 text-sm flex items-center justify-between">
          <span>
              ...
              <tbody className="divide-y divide-slate-100">
                {sorted.map(k => (
                  <tr key={k.id} className={selected.has(k.id) ? 'bg-indigo-50' : ''}>
                    ...
                    <td className="px-3 py-2 font-medium text-slate-800">{k.text}</td>
                    ...
                    <td className="px-3 py-2 text-slate-600">{fmtNum(k.competitors)}</td>
                    <td className="px-3 py-2 text-slate-600">{fmtNum(k.avg_pages)}</td>
                    <td className="px-3 py-2 text-slate-600">{fmtCurrency(k.avg_price)}</td>
                    <td className="px-3 py-2 text-slate-600">{fmtCurrency(k.avg_monthly_earnings)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
