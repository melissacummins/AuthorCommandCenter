...
export default function ProjectList({
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 lg:p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Audiobooks</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white rounded-2xl border border-slate-200">
          <AudioLines className="w-8 h-8 text-slate-300 mx-auto mb-2" />
          <p className="text-sm text-slate-500">No audiobooks yet. Name one above to get started.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {projects.map(p => (
            <div key={p.id} className="group flex items-center gap-3 p-4 bg-white rounded-xl border border-slate-200 hover:border-slate-300 hover:shadow-sm transition-all">
              ...
              <button onClick={() => onOpen(p.id)} className="text-slate-300 group-hover:text-slate-500"><ChevronRight className="w-5 h-5" /></button>
            </div>
          ))}
        </div>
      )}
