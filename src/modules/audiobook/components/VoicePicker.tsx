    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200">
          <h3 className="font-semibold text-slate-800">Assign a voice — <span className="text-violet-600">{roleLabel}</span></h3>
        ...
        <div className="flex gap-1 px-5 pt-3">
          {([['library', 'Pick from library', Library], ['design', 'Create from description', Wand2], ['clone', 'Clone from audio', Mic]] as const).map(([id, label, Icon]) => (
            <button key={id} onClick={() => setTab(id)}
              className={`inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg ${tab === id ? 'bg-violet-50 text-violet-700' : 'text-slate-500 hover:bg-slate-50'}`}>
...
function PreviewButton({ src }: { src: string | null }) {
  ...
  return (
    <>
      <button onClick={() => audioRef.current?.play()} className="inline-flex items-center gap-1 text-xs text-slate-500 hover:text-violet-600">
...
function LibraryTab({ onAssign }: { onAssign: (id: string, name: string) => void }) {
  ...
  return (
    <div>
      ...
      {shown.length === 0 && <p className="text-sm text-slate-400 py-4 text-center">No voices match.</p>}
      <div className="space-y-1.5">
        {shown.map(v => (
          <div key={v.voice_id} className="flex items-center gap-3 p-2.5 rounded-lg border border-slate-100 hover:border-slate-200">
            ...
            <button onClick={() => onAssign(v.voice_id, v.name)}
              className="px-3 py-1.5 text-xs font-medium text-white rounded-lg bg-violet-600 hover:bg-violet-700">
              Use
            </button>
          </div>
        ))}
...
function DesignTab({ onAssign }: { onAssign: (id: string, name: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      <div>
        <label className="block text-xs font-medium text-slate-500 mb-1">Describe the voice</label>
      ...
      <button onClick={generate} disabled={busy}
        className="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-white rounded-lg bg-violet-600 hover:bg-violet-700 disabled:opacity-50">
        {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />} Generate previews
      </button>

      {previews.length > 0 && (
        <div className="space-y-2 pt-1">
          <p className="text-xs font-medium text-slate-500">Pick the one you like, name it, and save:</p>
          {previews.map((p, i) => (
            <label key={p.generated_voice_id} className={`flex items-center gap-3 p-2.5 rounded-lg border cursor-pointer ${chosen === p.generated_voice_id ? 'border-violet-400 bg-violet-50' : 'border-slate-100'}`}>
              <input type="radio" name="preview" checked={chosen === p.generated_voice_id} onChange={() => setChosen(p.generated_voice_id)} />
              <span className="text-sm text-slate-700 flex-1">Preview {i + 1}</span>
  ...
  return (
    <div className="space-y-3">
      ...
      <input value={name} onChange={e => setName(e.target.value)} placeholder="Voice name (e.g. Marcus)"
        className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" />
      <label className="flex items-center justify-center gap-2 px-3 py-6 border-2 border-dashed border-slate-300 rounded-lg text-sm text-slate-500 cursor-pointer hover:border-violet-400">
        ...
        <ul className="space-y-1">
          {files.map((f, i) => (
            <li key={i} className="flex items-center gap-2 text-sm text-slate-600">
              ...
              <button onClick={() => setFiles(prev => prev.filter((_, j) => j !== i))} className="text-slate-400 hover:text-rose-600">
