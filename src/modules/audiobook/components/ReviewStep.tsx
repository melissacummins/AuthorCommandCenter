...
export default function ReviewStep({
  ...
  if (!segments.length) {
    return <p className="text-sm text-slate-400">No segments yet — analyze a manuscript in the first step.</p>;
  }
...
function SegmentRow({
  ...
  return (
    <div className="flex items-start gap-2 p-2.5 rounded-lg border border-slate-100">
      <div className="flex flex-col gap-1.5 w-32 shrink-0">
        <select value={segment.speaker} onChange={e => onUpdate(segment.id, { speaker: e.target.value as Speaker })}
          className={`px-2 py-1 text-xs font-medium rounded-md border ${SPEAKER_STYLE[segment.speaker]}`}>
        ...
        <input value={name} onChange={e => setName(e.target.value)}
          ...
          placeholder="name" className="px-2 py-1 text-xs border border-slate-200 rounded-md" />
      ...
      <textarea value={text} onChange={e => setText(e.target.value)}
        ...
        className="flex-1 px-2.5 py-1.5 text-sm border border-slate-200 rounded-md leading-relaxed" />
      <button onClick={() => { if (confirm('Delete this segment?')) onDelete(segment.id); }}
        className="text-slate-300 hover:text-rose-600 mt-1"><Trash2 className="w-4 h-4" /></button>
    </div>
  );
}
