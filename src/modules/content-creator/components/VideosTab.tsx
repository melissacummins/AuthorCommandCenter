    ...
    <div className="space-y-5 max-w-4xl">
        ...
        <div className="bg-surface rounded-card border border-edge p-5 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-semibold text-content flex items-center gap-2">
              <Video className="w-4 h-4 text-content-muted" /> Videos
            </h3>
      ...
      {creatives.length === 0 && !creating ? (
        <div className="bg-surface rounded-card border border-dashed border-edge-strong p-10 text-center">
          <p className="text-content-secondary text-sm">No videos yet for this book.</p>
        </div>
      ) : (
            ...
            return (
              <div key={c.id} className="bg-surface rounded-card border border-edge p-4 flex items-start justify-between gap-2">
                <button className="text-left min-w-0 flex-1" onClick={() => setOpenId(c.id)}>
                  <p className="text-sm font-medium text-content truncate">{c.title || 'Untitled video'}</p>
...
function NewVideoForm({ userId, book, approvedHooks, onCancel, onCreated }: {
  ...
  return (
    <div className="bg-surface rounded-card border border-edge p-5 space-y-4">
      <h3 className="text-sm font-semibold text-content">New video script</h3>
      <div>
      ...
      <div className="flex flex-wrap items-end gap-4">
        <div>
          <label className="block text-xs font-medium text-content-secondary mb-1">Target length</label>
  ...
  return (
    <div className="space-y-4 max-w-5xl">
      <div className="bg-surface rounded-card border border-edge p-3 flex flex-wrap items-center gap-2 text-xs">
        ...
        <span className="text-sm font-medium text-content truncate max-w-60">{creative.title}</span>
        <span className="text-content-muted">~{Math.round(dur)}s · 1080×1920</span>
        ...
        <button onClick={exportAssets} disabled={busy === 'assets'}
          className="px-3 py-2 rounded-control border border-edge-strong text-content font-medium hover:bg-surface-hover disabled:opacity-50 flex items-center gap-1.5">
      ...
      <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
        ...
        <div className="space-y-2">
          <div className="relative rounded-card overflow-hidden border border-edge bg-black" style={{ aspectRatio: '9 / 16' }}>
            {payload.bg_url ? (
              <video
                ref={videoRef}
                src={payload.bg_url}
              ...
              <div className="absolute inset-0 grid place-items-center text-content-secondary text-xs px-6 text-center">
          ...
          <div className="bg-surface rounded-card border border-edge p-3 space-y-2">
            <p className="text-xs font-medium text-content-secondary flex items-center gap-1.5"><Music className="w-3.5 h-3.5" /> Music</p>
            {payload.music_url ? (
              <div className="flex items-center gap-2 text-xs text-content-secondary">
                <audio src={payload.music_url} controls className="h-8 flex-1" />
                <button onClick={() => commit({ ...payload, music_url: null })} className="text-content-muted hover:text-rose-600"><X className="w-3.5 h-3.5" /></button>
              </div>
            ) : (
              <>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs text-content-secondary">
            ...
            <select value={payload.style.size} onChange={e => commit({ ...payload, style: { ...payload.style, size: e.target.value as SlideStyle['size'] } })}
              className="rounded border border-edge px-1.5 py-1 bg-surface"><option value="sm">S</option><option value="md">M</option><option value="lg">L</option></select>
            <select value={payload.style.position} onChange={e => commit({ ...payload, style: { ...payload.style, position: e.target.value as SlideStyle['position'] } })}
              className="rounded border border-edge px-1.5 py-1 bg-surface"><option value="top">Top</option><option value="middle">Middle</option><option value="bottom">Bottom</option></select>
            <select value={payload.style.color} onChange={e => commit({ ...payload, style: { ...payload.style, color: e.target.value as SlideStyle['color'] } })}
              className="rounded border border-edge px-1.5 py-1 bg-surface"><option value="white">White</option><option value="black">Black</option></select>
          ...
          {payload.lines.map((line, i) => (
            <div key={i} className={`bg-surface rounded-card border p-3 flex items-start gap-2 ${active === line ? 'border-pink-300' : 'border-edge'}`}>
              ...
              <button onClick={() => commit({ ...payload, lines: payload.lines.filter((_, j) => j !== i) })}
                className="p-1.5 text-content-faint hover:text-rose-600"><Trash2 className="w-3.5 h-3.5" /></button>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-6" onClick={() => setLibraryOpen(false)}>
          <div className="bg-surface rounded-card p-4 max-w-2xl w-full max-h-[70vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-semibold text-content">Video library</h4>
            ...
            {!library ? (
              ...
              <p className="text-sm text-content-muted py-6 text-center">No completed videos in your Media library — generate one in the Media module first.</p>
            ) : (
