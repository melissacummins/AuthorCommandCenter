    ...
    <div className="space-y-5 max-w-4xl">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-semibold text-slate-800 flex items-center gap-2">
              <GalleryHorizontalEnd className="w-4 h-4 text-slate-400" /> Slideshows
            </h3>
      ...
      {creatives.length === 0 && !creating ? (
        <div className="bg-white rounded-xl border border-dashed border-slate-300 p-10 text-center">
          <p className="text-slate-500 text-sm">No slideshows yet for this book.</p>
        </div>
      ) : (
            ...
            return (
              <div key={c.id} className="bg-white rounded-xl border border-slate-200 p-4 flex items-start justify-between gap-2">
                <button className="text-left min-w-0 flex-1" onClick={() => setOpenId(c.id)}>
                  <p className="text-sm font-medium text-slate-800 truncate">{c.title || 'Untitled slideshow'}</p>
...
function NewSlideshowForm({ userId, book, approvedHooks, onCancel, onCreated }: {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
      <h3 className="text-sm font-semibold text-slate-800">New slideshow</h3>
      <div>
      ...
      <div className="flex flex-wrap gap-4 items-end">
        <div>
          <label className="block text-xs font-medium text-slate-600 mb-1">Slides</label>
    ...
    <div className="inline-flex rounded-lg border border-slate-200 overflow-hidden">
      {(Object.keys(SLIDE_FORMATS) as SlideFormat[]).map(f => (
        <button key={f} onClick={() => onChange(f)} title={SLIDE_FORMATS[f].hint}
          className={`px-3 py-2 text-xs font-medium ${value === f ? 'bg-pink-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}>
    ...
    <div className="space-y-4 max-w-5xl">
      <div className="bg-white rounded-xl border border-slate-200 p-4 flex flex-wrap items-center gap-3">
        ...
        <input
          ...
          className="flex-1 min-w-40 text-sm font-medium text-slate-800 bg-transparent border-0 focus:outline-none"
        />
        ...
        <span className="text-[11px] text-slate-400 w-14">{saveState === 'saving' ? 'Saving…' : 'Saved'}</span>
        <button onClick={exportAll} disabled={exporting}
          className="px-3 py-2 rounded-lg bg-slate-800 text-white text-xs font-medium hover:bg-slate-700 disabled:opacity-50 flex items-center gap-1.5">
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
        ...
        <span className="text-slate-400">billed to your Fal/OpenAI key, same as the Media module</span>
      </div>
      {error && <p className="text-xs text-rose-600">{error}</p>}
...
function SlideRow({ index, total, slide, format, guides, bannedActive, imageModel, userId, sceneExcerpt, onPatch, onMove, onDelete }: {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4 flex gap-4">
      {/* Preview */}
      <div
        className="relative shrink-0 rounded-lg overflow-hidden border border-slate-200"
        style={{
      ...
      <div className="flex-1 min-w-0 space-y-2">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <button onClick={generateBg} disabled={bgBusy} className="flex items-center gap-1 text-slate-600 hover:text-pink-600 disabled:opacity-50">
            {bgBusy ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Wand2 className="w-3.5 h-3.5" />} Generate bg
          </button>
          <button onClick={openLibrary} className="flex items-center gap-1 text-slate-600 hover:text-pink-600">
          ...
          {slide.bg_url && (
            <button onClick={() => onPatch({ bg_url: null })} className="flex items-center gap-1 text-slate-400 hover:text-rose-600">
        ...
        <div className="flex items-center gap-1.5 pt-1">
          <button onClick={() => onMove(-1)} disabled={index === 0} className="p-1 rounded text-slate-400 hover:text-slate-700 disabled:opacity-30"><ArrowUp className="w-3.5 h-3.5" /></button>
          <button onClick={() => onMove(1)} disabled={index === total - 1} className="p-1 rounded text-slate-400 hover:text-slate-700 disabled:opacity-30"><ArrowDown className="w-3.5 h-3.5" /></button>
          <button onClick={() => navigator.clipboard.writeText(slide.text)} title="Copy text" className="p-1 rounded text-slate-400 hover:text-slate-700"><Copy className="w-3.5 h-3.5" /></button>
          <button
          ...
          <button onClick={onDelete} className="p-1 rounded text-slate-300 hover:text-rose-600 ml-auto"><Trash2 className="w-3.5 h-3.5" /></button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-6" onClick={() => setLibraryOpen(false)}>
          <div className="bg-white rounded-xl p-4 max-w-2xl w-full max-h-[70vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-semibold text-slate-800">Media library</h4>
            ...
            {!library ? (
              ...
              <p className="text-sm text-slate-400 py-6 text-center">No completed images in your Media library yet.</p>
            ) : (
              <div className="grid grid-cols-4 gap-2">
                {library.map(img => (
                  <button key={img.id} title={img.prompt}
                    onClick={() => { onPatch({ bg_url: img.url }); setLibraryOpen(false); }}
                    className="aspect-square rounded-lg overflow-hidden border border-slate-200 hover:ring-2 hover:ring-pink-400">
