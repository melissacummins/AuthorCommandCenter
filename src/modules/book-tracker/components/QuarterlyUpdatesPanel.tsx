...
export default function QuarterlyUpdatesPanel({ book, updates, onAdd, onDelete }: Props) {
  ...
  return (
    <div className="bg-surface rounded-card border border-edge p-5 space-y-4">
      <div>
        <h3 className="font-semibold text-content">Quarterly updates</h3>
      ...
      <form onSubmit={handleSubmit} className="flex flex-wrap gap-2 items-end">
        <div>
          <label className="block text-xs font-medium text-content-secondary mb-1">Quarter</label>
          <input
            type="text"
            value={label}
            onChange={e => setLabel(e.target.value)}
            className="px-3 py-1.5 border border-edge-strong rounded-control text-sm w-32"
            placeholder="Q4 2024"
          />
        </div>
        <div>
          <label className="block text-xs font-medium text-content-secondary mb-1">Profit</label>
          <div className="relative">
            ...
            <input
              ...
              className="pl-6 pr-2 py-1.5 border border-edge-strong rounded-control text-sm w-32"
              placeholder="0.00"
            />
          </div>
        </div>
      ...
      {sorted.length > 0 ? (
        <div className="border-t border-edge pt-3">
          <div className="grid grid-cols-[1fr_auto_auto] gap-x-3 gap-y-1 text-sm">
            {sorted.map(u => (
              <div key={u.id} className="contents">
                <div className="text-content">{u.quarter_label}</div>
                <div className="text-content font-medium tabular-nums">${Number(u.profit).toFixed(2)}</div>
                <button
                  onClick={() => onDelete(u.id)}
                  className="text-content-muted hover:text-rose-600 px-1"
          ...
          <div className="border-t border-edge mt-2 pt-2 flex justify-between text-sm">
            <span className="text-content-secondary">Cumulative</span>
            <span className="font-semibold text-content">${book.cumulative_profit.toFixed(2)}</span>
          </div>
        </div>
      ) : (
        <p className="text-sm text-content-muted italic">No updates yet.</p>
      )}
    </div>
  );
}
