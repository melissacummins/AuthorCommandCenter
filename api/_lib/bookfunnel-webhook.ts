// Shared BookFunnel webhook handler, used by both route files:
//   api/bookfunnel/webhook.ts        (…/api/bookfunnel/webhook?u=&t=)  — legacy
//   api/bookfunnel/[u]/[t].ts        (…/api/bookfunnel/<user>/<secret>) — primary
//
// BookFunnel drops query strings when it POSTs, so the path-based URL carries
// the user id + secret in the PATH. Vercel exposes both query params and path
// segments on req.query, so this single handler serves both shapes. The route
// files own `export const config` (Vercel reads bodyParser config from the route
// file, not from imports) and delegate here.
//
// We don't yet know BookFunnel's exact payload shape, so we store the FULL raw
// body and only best-effort extract email / name / page / book.
//
...
// Any extra params on the URL (one PARAMS-mode variant), minus our auth params.
...
// Our secret is 64 hex chars. If a sender appends data and corrupts the `t`
// value (e.g. "<secret>?email=…"), pull the secret back out so it still validates.
