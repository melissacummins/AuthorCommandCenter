// Set / get / remove the caller's OpenAI API key. Stored encrypted
// with AES-256-GCM using the same FAL_KEY_ENCRYPTION_SECRET master
// secret as the Fal key — no new env var needed. Decryption only ever
// happens inside server-side handlers.
//
// Endpoints:
//   GET    /api/media/openai-key                    — returns { has_key, hint }
//   POST   /api/media/openai-key  { key: string }   — encrypts and stores
//   DELETE /api/media/openai-key                    — removes the user's key
...
// Same salt as the Fal key so we can reuse the FAL_KEY_ENCRYPTION_SECRET
// master without giving operators a second secret to manage. The salt
// is per-key-type so the derived AES key still differs.
function deriveMasterKey(secret: string): Buffer {
  return scryptSync(secret, 'media-openai-key-v1', 32);
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'DELETE') {
    await supabase.from('user_openai_keys').delete().eq('user_id', userId);
  ...
  if (rawKey.length < 20 || !rawKey.startsWith('sk-')) {
    res.status(400).json({ error: 'OpenAI keys start with "sk-" — paste the full secret key from platform.openai.com.' });
    return;
  }
